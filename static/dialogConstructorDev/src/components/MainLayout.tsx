...
const getListOfGraphs = async () => {
    ...
    return listOfNodes?.graph_list || [{ id: 1, title: 'title' }, { id: 2, title: '213123' }];
};
...
export const MainLayout = ({ app }: MainLayoutProps) => {
    ...
    const saveGraph = ({ id, title }: { id: string, title: string }) => {
        ...
        const momChild = arrowsDates.map(m => {
            return { mom: m.source, child: m.target, childName: names.find(name => name.id === m.target).name };
        });
        const graph = nodeDates.map(node => {
            return {
                ...
                node_links: momChild.filter(item => item.mom === node.id).map(i => i.childName),
                node_views: 0,
            };
        });
        const result = {
    ...
    return (
        <div>
            ...
            <DialogConstructorHeader>
                <select name="dialogs" onChange={(e) => {
                ...
                <AddDialogButton onClick={() => saveGraph({ ...selectGraph })}>Сохранить граф</AddDialogButton>
            </DialogConstructorHeader>
